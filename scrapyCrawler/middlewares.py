...
class CustomDownloaderMiddleware(object):
    ...
    def process_response(self, request, response, spider):
        ...
        contype = response.headers.get(b'content-type', None)
        if contype == None:
            return response
            
        if 'text' in str(contype):
            return response
        else:
            msg = "Ignoring request {}, content-type was {}(not 'text/html)'".format(response.url, str(contype))
